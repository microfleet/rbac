...
export interface ConfigurationIAP {
  keys: KeyIAP[]
  secrets: SecretIAP[]
...
export class IAP {
  private readonly config: ConfigurationIAP
  private readonly keys: {
    [kid: string]: KeyIAP,
  }
  private readonly secrets: {
    [sid: string]: SecretIAP,
  }

  constructor(opts: ConfigurationIAP) {
    this.config = opts
   ...
   */
  public async verifyRequest(payload: ObjectLike, opts?: VerifyOptions): Promise<string> {
   ...
   */
  public async verifyProxy(payload: ObjectLike, opts?: VerifyOptions): Promise<string> {
    ...
    const sub = (decoded as EncodedPayload).sub
    assert(sub && typeof sub === 'string', kJWTNoUserId)

    return sub
  }

  private resolveAsyncKey(header: any, key: KeyIAP | void, callback: Callback) {
    if (key === undefined) {
  ...
  private resolveSharedKey(secret: SecretIAP | void, callback: Callback) {
    if (secret === undefined) {
      return callback(kSecretNotFound)
    }
  ...
  private resolveSecret(header: any, callback: Callback) {
    ...
    if (typeof header.sid === 'string' && header.alg && header.alg.startsWith('HS')) {
